...
static void PlaceAirport(uint tile)
{
	...
	ret = DoCommandByTile(tile, _selected_airport_type, 0, 
		DC_EXEC | DC_AUTO | DC_NO_WATER | DC_MSG(STR_A001_CAN_T_BUILD_AIRPORT_HERE),
		CMD_BUILD_AIRPORT);
...
{
	HandlePlacePushButton(w, 3, ANIMCURSOR_DEMOLISH, 1, PlaceProc_Demolish);
}
...
static void BuildAirportPickerWndProc(Window *w, WindowEvent *e)
{
	...
	case WE_PAINT: {
		...
		if (!(_avail_aircraft & 4)) {	w->disabled_state |= (1<<5); }
		_selected_airport_type = sel;
		w->click_state = ((1<<3) << sel) | ((1<<6) << _station_show_coverage);
		...
		DrawStringCentered(74, 53, STR_3066_COVERAGE_AREA_HIGHLIGHT, 0);
		DrawStationCoverageAreaText(2, 77, (uint)-1);
		break;
	}
	...
	case WE_CLICK: {
		...
		case 6:
		case 7:
			_station_show_coverage = e->click.widget - 6;
			SndPlayFx(0x13);
			SetWindowDirty(w);
			break;
		}
...
{      WWT_PANEL,     7,     0,   147,    14,   107, 0x0,													0},
...
{WWT_NODISTXTBTN,    14,     2,   145,    39,    50, STR_306B_HELIPORT,						STR_3058_SELECT_SIZE_TYPE_OF_AIRPORT},
{   WWT_CLOSEBOX,    14,    14,    73,    62,    73, STR_02DB_OFF,								STR_3065_DON_T_HIGHLIGHT_COVERAGE},
{   WWT_CLOSEBOX,    14,    74,   133,    62,    73, STR_02DA_ON,									STR_3064_HIGHLIGHT_COVERAGE_AREA},
{      WWT_LAST},
};
