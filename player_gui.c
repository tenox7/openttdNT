...
static void PlayerFinancesWndProc(Window *w, WindowEvent *e)
{
	...
	case WE_CLICK:
		...
		case 5: /* increase loan */
			DoCommandByTile(0, w->window_number, _ctrl_pressed, DC_EXEC | DC_MSG(STR_702C_CAN_T_BORROW_ANY_MORE_MONEY), CMD_INCREASE_LOAN);
			break;

		case 6: /* repay loan */
			DoCommandByTile(0, w->window_number, _ctrl_pressed, DC_EXEC | DC_MSG(STR_702F_CAN_T_REPAY_LOAN), CMD_DECREASE_LOAN);
			break;
		}
		break;
	}
		...
		if (e->click.widget == 2) {
			...
			for(i=0; i!=16; i++) {
				if (!(used_colors & 1) && --item < 0) {
					DoCommandByTile(0, w->window_number, i, DC_EXEC, CMD_SET_PLAYER_COLOR);
					DeleteWindow(w);
					break;
				}
				used_colors >>= 1;
...
static void SelectPlayerFaceWndProc(Window *w, WindowEvent *e)
{
	...
	case WE_CLICK:
		...
		case 4: /* ok click */ 
			DoCommandByTile(0, w->window_number, WP(w,facesel_d).face, DC_EXEC, CMD_SET_PLAYER_FACE);
			DeleteWindow(w);
			break;
		case 5: /* male click */
		case 6: /* female click */
...
static void DrawPlayerVehiclesAmount(int player)
{
	...
	if (train+road+air+ship == 0) {
		...
		if (ship != 0) {
			SET_DPARAM16(0, ship);
			DrawString(x + 70, y, ship==1 ? STR_7040_SHIP : STR_703F_AIRCRAFT, 0);
		}
	}
}
...
static void PlayerCompanyWndProc(Window *w, WindowEvent *e)
{
	...
	case WE_CLICK:
		...
		case 8: /* buy 25% */
			DoCommandByTile(0, w->window_number, 0, DC_EXEC | DC_MSG(STR_707B_CAN_T_BUY_25_SHARE_IN_THIS), CMD_BUY_SHARE_IN_COMPANY);
			break;

		case 9: /* sell 25% */
			DoCommandByTile(0, w->window_number, 0, DC_EXEC | DC_MSG(STR_707C_CAN_T_SELL_25_SHARE_IN), CMD_SELL_SHARE_IN_COMPANY);
			break;
		}
		break;
	...
	case WE_PLACE_OBJ:
		if (DoCommandByTile(e->place.tile, 0, 0, DC_EXEC | DC_AUTO | DC_NO_WATER | DC_MSG(STR_7071_CAN_T_BUILD_COMPANY_HEADQUARTERS), CMD_BUILD_COMPANY_HQ) != CMD_ERROR)
			ResetObjectToPlace();
		SetWindowDirty(w);
		break;
	...
	case WE_ON_EDIT_TEXT: {
		...
		if (WP(w,def_d).byte_1) {
			DoCommandByTile(0, w->window_number, 0, DC_EXEC | DC_MSG(STR_700C_CAN_T_CHANGE_COMPANY_NAME), CMD_CHANGE_COMPANY_NAME);
		} else {
			DoCommandByTile(0, w->window_number, 0, DC_EXEC | DC_MSG(STR_700D_CAN_T_CHANGE_PRESIDENT), CMD_CHANGE_PRESIDENT_NAME);
		}
	} break;
...
static void BuyCompanyWndProc(Window *w, WindowEvent *e)
{
	...
	case WE_CLICK:
		...
		case 3:
			...
			break;
		case 4: {
			int wnum = w->window_number;	
			if (DoCommandByTile(0, wnum, 0, DC_EXEC | DC_MSG(STR_7060_CAN_T_BUY_COMPANY),CMD_BUY_COMPANY) != CMD_ERROR)
				DeleteWindowById(WC_BUY_COMPANY, wnum);
			break;
		}
		}
		break;
