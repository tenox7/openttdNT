...
void InitializeVehicles()
{
	...
	// setup indexes..
	for(v = _vehicles, i = 0; v != endof(_vehicles); v++)
		v->index = i++;
...
Vehicle *GetPrevVehicleInChain(Vehicle *v)
{
	...
	v = _vehicles;
	while (v->type != VEH_Train || veh != v->next_in_chain) {
...
{
	int count = 0;
	while (++count, v->next_in_chain != INVALID_VEHICLE) {
		v = &_vehicles[v->next_in_chain];
	}
	return count;
}
...
{
	return CreateEffectVehicle(x, y, (byte)GetSlopeZ(x, y) + z, type);
}
...
void CheckClickOnVehicle(ViewPort *vp, int x, int y)
{
	...
	if (found != NULL)
		_on_vehicle_click_proc[found->type - 0x10](found);
}
...
const byte _common_veh_desc[] = {
	...
	SLE_VAR(Vehicle,next_in_chain,    SLE_UINT16),
...
static const byte _aircraft_desc[] = {
	...
	SLE_VARX(offsetof(Vehicle,u)+offsetof(VehicleAir,unk0),							SLE_UINT8),
...
static const byte _depot_desc[] = {
	...
	SLE_VAR(Depot,city_index,		SLE_UINT16),
	SLE_END()
};
...
static const byte _checkpoint_desc[] = {
	...
	SLE_VAR(Checkpoint,city_or_string,		SLE_UINT16),
