...
static void LandInfoWndProc(Window *w, WindowEvent *e)
{
	...
	if (e->event == WE_PAINT) {
		...
		if (lid->td.owner != 0x10 && lid->td.owner != 0x11)
			GetNameOfOwner(lid->td.owner);
		...
		if (lid->city != NULL) {
			SET_DPARAM16(0, lid->city->citynametype);
			SET_DPARAM32(1, lid->city->citynameparts);
		}
		DrawStringCentered(140,49, STR_01A8_LOCAL_AUTHORITY, 0);
		...
		if (str != (STR_01CE_CARGO_ACCEPTED - 1))
			DrawStringMultiCenter(140, 65, str, 276);
	}
}
...
{     WWT_IMGBTN,    14,     0,   279,    14,    81, 0x0, 0},
{      WWT_LAST},
};
...
//	if (IS_TILETYPE(tile, MP_STRANGE))
//		return;
	...
	lid.city = ClosestCityFromTile(tile, 20);
...
static void AboutWindowProc(Window *w, WindowEvent *e)
{
	...
	case WE_PAINT: {
		...
		x = 0xc8;
		y = 0;
		DrawStringCentered(x, y+=0x11, STR_00B6_COPYRIGHT_1995_CHRIS_SAWYER, 0);
...
static const WindowDesc _about_desc = {
	WDP_CENTER, WDP_CENTER, 0x190, 0x110,
	WC_GAME_OPTIONS,0,
	WDF_STD_TOOLTIPS | WDF_STD_BTN | WDF_DEF_WIDGET,
	_about_widgets,
	AboutWindowProc
...
static void BuildTreesWndProc(Window *w, WindowEvent *e)
{
	...
	case WE_PLACE_OBJ:
		DoCommandByTile(e->place.tile, _tree_to_plant, 0, 
			DC_EXEC | DC_AUTO | DC_MSG(STR_2805_CAN_T_PLANT_TREE_HERE),
			CMD_PLANT_TREE);
		break;
...
{      WWT_PANEL,     7,     0,   142,    14,   157, 0x0,							0},
...
{      WWT_PANEL,    14,    72,   105,   110,   155, 0x0,							STR_280D_SELECT_TREE_TYPE_TO_PLANT},
{      WWT_PANEL,    14,   107,   140,   110,   155, 0x0,							STR_280D_SELECT_TREE_TYPE_TO_PLANT},
{   WWT_CLOSEBOX,    14,     2,   140,   157,   168, STR_028A_RANDOM_TREES,STR_028B_PLANT_TREES_RANDOMLY_OVER},
{      WWT_LAST},
};
...
void ShowErrorMessage(StringID msg_1, StringID msg_2, int x, int y)
{
	...
	if (_errmsg_message_1 != STR_013B_OWNED_BY || GET_DPARAMX8(_errmsg_decode_params,2) >= 8) {
		...
		if ( (x|y) != 0) {
			...
			vp = w->viewport;
			pt.x = clamp(((pt.x - vp->virtual_left) >> vp->zoom) + vp->left - (240/2), 0, _screen.width - 240);
			pt.y = clamp(((pt.y - 48 - vp->virtual_top) >> vp->zoom) + vp->top - (46/2), 22, _screen.height - 46);
		} else {
...
void CheckRedrawStationCoverage(Window *w)
{
	...
	x = _thd.pos.x;
	y = _thd.pos.y;
	if (_thd.drawstyle != HT_RECT) x = -1;

	if (x != _thd.last_station_pos.x || y != _thd.last_station_pos.y) {
		_thd.last_station_pos.x = x;
		_thd.last_station_pos.y = y;
		SetWindowDirty(w);
	}
}
...
{     WWT_IMGBTN,    14,     0,   256,    14,    35, 0x0},
{     WWT_IMGBTN,    14,     0,   245,    36,   291, 0x0},
{     WWT_IMGBTN,    14,     2,   243,    38,   289, 0x0, STR_400A_LIST_OF_DRIVES_DIRECTORIES},
{  WWT_SCROLLBAR,    14,   246,   256,    36,   291, 0x0, STR_0190_SCROLL_BAR_SCROLLS_LIST},
{      WWT_LAST},
};
...
static const FiosItem *_fios_list;
...
static void DrawFiosTexts()
{
	...
	if (str != 0)
		DrawString(2, 25, str, 0);
	DoDrawString(path, 2, 15, 16);
}

static const byte _fios_colors[] = {13, 9, 9, 6, 5};
...
static void SaveLoadDlgWndProc(Window *w, WindowEvent *e)
{
	...
	case WE_PAINT: {
		...
		GfxFillRect(3, 39, 3+239, 39+249, 0xD7);

		y = 39;
		pos = w->vscroll.pos;
		while (pos < _fios_num) {
			item = _fios_list + pos;
			DoDrawString(item->name, 4, y, _fios_colors[item->type] );
			pos++;
			y+=10;
			if (y >= 25*10+39)
				break;
	...
	case WE_CLICK:
		switch(e->click.widget) {
		case 4: {
			int y = (e->click.pt.y - 39) / 10;
			char *name;

			if (y < 0 || (y += w->vscroll.pos) >= w->vscroll.count)
				return;
			
			if ((name = FiosBrowseTo(_fios_list + y)) != NULL) {
				if (_saveload_mode == SLD_LOAD_GAME) {
					_switch_mode = SM_LOAD;
					DeleteWindow(w);
					strcpy(_file_to_saveload, name);
				} else if (_saveload_mode == SLD_LOAD_SCENARIO) {
					_switch_mode = SM_LOAD;
					DeleteWindow(w);
					strcpy(_file_to_saveload, name);
				} else {
					strcpy(WP(w,querystr_d).buf, _fios_list[y].name);
	...
	case WE_TIMEOUT:
		if (HASBIT(w->click_state, 8)) {
			...
			SetWindowDirty(w);
			BuildFileList();
		} else if (HASBIT(w->click_state, 9)) {
			// Save button clicked
			_switch_mode = SM_SAVE;
			FiosMakeSavegameName(_file_to_saveload, WP(w,querystr_d).buf);
		}
		break;
	case WE_DESTROY:
		UnpauseGame();
...
static const WindowDesc _load_dialog_desc = {
	WDP_CENTER, WDP_CENTER, 0x101, 0x124,
	WC_SAVELOAD,0,
