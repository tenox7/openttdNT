...
void DispatchLeftClickEvent(Window *w, int x, int y) {
	...
	if (w->desc_flags & WDF_DEF_WIDGET) {
		...
		if (w->desc_flags & WDF_STD_BTN) {
			...
			else if (e.click.widget == 1) StartWindowDrag(w);
		}
	} else {
		w->wndproc(w, &e);
	}
...
//	w->unk22 = 0xFFFF;
	...
	memset(w->custom, 0, sizeof(w->custom));
...
stop_capt:;
	...
	if (w->window_class != WC_SMALLMAP) {
		...
		if (x < 16) { x = 16; sub = 0; }
		if (x > 4064) { x = 4064; sub = 0; }
		if (y < -1120) { y = -1120; sub = 0; }
		if (y > 3440) { y = 3440; sub = 0; }
	...
	if (click == 0) {
		if (_patches.autoscroll && _game_mode != GM_MENU) {
			...
			if (vp) {
				x -= vp->left;
				y -= vp->top;
				if (x-15<0) { WP(w,vp_d).scrollpos_x += (x-15) << vp->zoom; }
				else if (15-(vp->width-x) > 0) { WP(w,vp_d).scrollpos_x += (15-(vp->width-x)) << vp->zoom; }
			}
		}
		return;
	}
...
void RelocateAllWindows(int neww, int newh)
{
	...
	for(w=_windows; w!= _last_window ;w++) {
		...
		if (w->window_class == WC_SELECT_GAME || w->window_class == WC_GAME_OPTIONS){	
			top = (newh - w->height) >> 1;
			left = (neww - w->width) >> 1;
