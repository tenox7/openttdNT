...
{
	HandlePlacePushButton(w, 4, ANIMCURSOR_DEMOLISH, 1, PlaceProc_Demolish);
}
	...
	case WE_PLACE_MOUSEUP:
		if (e->place.pt.x != -1) {
			if (e->place.userdata == 0)
				...
				uint start_tile = TILE_FROM_XY(_thd.selstart.x, _thd.selstart.y);
				uint end_tile = TILE_FROM_XY(e->place.pt.x, e->place.pt.y);
				if (_remove_button_clicked) {
					if (DoCommandByTile(end_tile, start_tile, _place_road_flag, 
							DC_AUTO | DC_NO_WATER | DC_EXEC | DC_MSG(STR_1805_CAN_T_REMOVE_ROAD_FROM), CMD_REMOVE_LONG_ROAD) != CMD_ERROR) {
