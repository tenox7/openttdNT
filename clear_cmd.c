...
static int TerraformProc(TerraformerState *ts, uint tile, int mode)
{
	...
	if ((_map_type_and_height[tile] >> 4) == MP_RAILWAY) {
		...
		if (_map5[tile] == _railway_modes[mode])
			return 0;
	}
...
static bool TerraformTileHeight(TerraformerState *ts, uint tile, int height)
{
...

	{
		...
		static const int16 _terraform_tilepos[5] = {1, -0x2, 0x101, -0x200, 0 };
...
int32 ClearTile_Clear(uint tile, byte flags) {
	...
	price = _clear_price_table[_map5[tile] & 0x1F];
	if (price == NULL)
		return 0;
	return *price;
}
...
uint16 GetSlopeZ_Clear(TileInfo *ti) { return GetPartialZ(ti->x&0xF, ti->y&0xF, ti->tileh) + ti->z; }
...
void TileLoopClearHelper(uint tile)
{
	...
	img_2 = 0;
	if (IS_TILETYPE(tile+1, MP_CLEAR)) {
		img_2 = img_by_map5[(_map5[tile+1] & 0x1C) >> 2];
	} else if (IS_TILETYPE(tile+1, MP_TREES) && (_map2[tile+1] & 0x30) == 0x20) {
		img_2 = 1;
	}
	...
	img_2 = 0;
	if (IS_TILETYPE(tile+0x100, MP_CLEAR)) {
		img_2 = img_by_map5[(_map5[tile+0x100] & 0x1C) >> 2];
	} else if (IS_TILETYPE(tile+0x100, MP_TREES) && (_map2[tile+0x100] & 0x30) == 0x20) {
		img_2 = 1;
	}
