...
static void RoadVehDetailsWndProc(Window *w, WindowEvent *e)
{
	...
	case WE_PAINT:
		...
		SET_DPARAM16(0, _engine_name_strings[v->engine_type]);
...
change_int:
			...
			if (!IS_INT_INSIDE(mod, 30, 800+1))
				return;
			DoCommandByTile(v->tile, v->index, mod,
				DC_EXEC | DC_MSG(STR_018A_CAN_T_CHANGE_SERVICING), CMD_CHANGE_ROADVEH_SERVICE_INT);
			break;
		}
		break;
	...
	case WE_ON_EDIT_TEXT: {
		...
		DoCommandByTile(0, w->window_number, 0, DC_EXEC | DC_MSG(STR_902D_CAN_T_NAME_ROAD_VEHICLE), CMD_NAME_VEHICLE);
	} break;
...
static void RoadVehViewWndProc(Window *w, WindowEvent *e)
{
	...
	case WE_CLICK: {
		...
		switch(e->click.widget) {
		case 4: /* start stop */
			DoCommandByTile(v->tile, v->index, 0,
				DC_EXEC | DC_MSG(STR_9015_CAN_T_STOP_START_ROAD_VEHICLE), CMD_START_STOP_ROADVEH); 
			break;
		...
		case 6: /* goto hangar */
			DoCommandByTile(v->tile, v->index, 0,
				DC_EXEC | DC_MSG(STR_9018_CAN_T_SEND_VEHICLE_TO_DEPOT), CMD_SEND_ROADVEH_TO_DEPOT);
			break;
		case 7: /* turn around */
			DoCommandByTile(v->tile, v->index, 0,
				DC_EXEC | DC_MSG(STR_9033_CAN_T_MAKE_VEHICLE_TURN), CMD_TURN_ROADVEH);
			break;
		case 8: /* show orders */
			ShowOrdersWindow(v);
			break;
		...
		do {
			if (HASBIT(e->player_avail, _local_player)) {
				...
				if (IS_INT_INSIDE(--pos, -8, 0)) {
					DrawString(x+59, y+2, _engine_name_strings[engine_id], sel==0 ? 0xC : 0x10);
...
static void NewRoadVehWndProc(Window *w, WindowEvent *e)
{
	...
	case WE_CLICK:
		...
		case 5: { /* build */
			...
			if (DoCommandByTile(w->window_number, sel_eng, 0,
				DC_EXEC | DC_MSG(STR_9009_CAN_T_BUILD_ROAD_VEHICLE), CMD_BUILD_ROAD_VEH) == CMD_ERROR)
					return;
		...
		case 6: /* rename */
			...
			ShowQueryString(
				_engine_name_strings[WP(w,buildtrain_d).sel_engine],
	...
	case WE_ON_EDIT_TEXT: {
		...
		if (*b == 0)
			return;
		memcpy(_decode_parameters, b, 32);
		DoCommandByTile(0, WP(w,buildtrain_d).rename_engine, 0, DC_EXEC | DC_MSG(STR_9037_CAN_T_RENAME_ROAD_VEHICLE), CMD_RENAME_ENGINE);
	} break;
...
static void RoadDepotClickVeh(Window *w, int x, int y)
{
	...
	switch (GetVehicleFromRoadDepotWndPt(w, x, y, &v)) {
	case 1:
		return;
	...
	case WE_DRAGDROP: {
		...
		case 3:
			if (!HASBIT(w->disabled_state, 3) &&
				...
				if (DoCommandByTile(v->tile, v->index, 0, 
					DC_EXEC | DC_MSG(STR_9014_CAN_T_SELL_ROAD_VEHICLE), CMD_SELL_ROAD_VEH) == CMD_ERROR)
						_backup_orders_tile = 0;
			}
			break;
		default:
