...
# define GET_WHEEL_DELTA_WPARAM(wparam) ((short)HIWORD(wparam))
	...
	case WM_MOUSEWHEEL: {
		...
		if (delta < 0) {
			ZoomInOrOutToCursor(false);
		} else if (delta > 0) {
			ZoomInOrOutToCursor(true);
		}
		return 0;
	}
...
static int Win32GdiMainLoop()
{
	...
	while(true) {
		...
		if (_exit_game)
			return ML_QUIT;
		if (_wnd.switch_driver)
			return ML_SWITCHDRIVER;

		if (_wnd.has_focus && GetAsyncKeyState(VK_SHIFT) < 0 || (cur_ticks=GetTickCount()) > next_tick) {
			next_tick = cur_ticks;
		} else {
			int d = next_tick - cur_ticks;
			if (d > 0) {
				Sleep(d);
			}
		}
...
static char *SubmitCrashReport(HWND wnd, void *msg, size_t msglen, const char *arg)
{
	return "crash reporting not available";
}
static void SubmitFile(HWND wnd, const char *file) {}
#endif
...
static int CDECL compare_FiosItems (const void *a, const void *b) {
...
const FiosItem *FiosGetSavegameList(int *num, int mode)
{
	...
	if (_fios_path == NULL) {
		...
		GetCurrentDirectory(MAX_PATH, _fios_path);
	}
	...
	if (_fios_path[3] != 0) {
		...
		strcpy(fios->name, ".. (Parent directory)");
	}
	
	hand = MyFindFirstFile(_fios_path, "*.*", &fd);
	if (hand != INVALID_HANDLE_VALUE) {
		do {
			if (fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY &&
				...
				sprintf(fios->name, "\\%s (Directory)", fd.cFileName);
			}
	...
	hand = MyFindFirstFile(_fios_path, "*.SAV", &fd);
	if (hand != INVALID_HANDLE_VALUE) {
		do {
			if (!(fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
				fios = FiosAlloc();
	...
	if(mode == SLD_LOAD_GAME || mode == SLD_LOAD_SCENARIO) {
		hand = MyFindFirstFile(_fios_path, "*.sv1", &fd);
		if (hand != INVALID_HANDLE_VALUE) {
			do {
				if (!(fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
					fios = FiosAlloc();
...
char *FiosBrowseTo(const FiosItem *item)
{
	...
	case FIOS_TYPE_OLDFILE:
		sprintf(str_buffr, "%s\\%s.sv1", _fios_path, item->name);
		return str_buffr;
	}
...
#endif
// testing 
// must be removed when sound code and effect are disabled
