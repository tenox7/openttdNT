...
static void SdlClose(uint32 x)
{
	...
	if (--_sdl_usage == 0) SDL_CALL SDL_Quit();
}
...
static void UpdatePalette(uint start, uint end) {
	...
	SDL_CALL SDL_SetColors(sdl_hwscreen, pal, start, end);
}
...
static void DrawSurfaceToScreen()
{
	...
	if ((n=_num_dirty_rects) != 0) {
		...
		if (n > MAX_DIRTY_RECTS)
			SDL_CALL SDL_UpdateRect(sdl_hwscreen, 0, 0, 0, 0);
		else {
			SDL_CALL SDL_UpdateRects(sdl_hwscreen, n, _dirty_rects);
		}
	}
}
...
static bool CreateMainSurface(int w, int h)
{
	...
	newscreen = SDL_CALL SDL_SetVideoMode(w, h, 8, SDL_SWSURFACE + (_fullscreen?SDL_FULLSCREEN:0));
	if(newscreen == NULL)
		return false;

	sdl_hwscreen = newscreen;
	InitPalette();
...
{
	SDL_CALL SDL_FreeSurface(sdl_hwscreen);
...
static int SdlVideoMainLoop()
{
	...
	while (true) {
		...
		_ctrl_pressed = !!(SDL_CALL SDL_GetModState() & (KMOD_LCTRL | KMOD_RCTRL));

		GameLoop();

		SDL_CALL SDL_LockSurface(sdl_hwscreen);
		_screen.dst_ptr = sdl_hwscreen->pixels;
		UpdateWindows();
		SDL_CALL SDL_UnlockSurface(sdl_hwscreen);

		if ((cur_ticks=SDL_CALL SDL_GetTicks()) > next_tick) {
			next_tick = cur_ticks;
			DEBUG(misc, 0) ("frameskip");
		} else {
			DrawSurfaceToScreen();
		}

		if (!(SDL_CALL SDL_GetModState() & KMOD_LSHIFT)) {
