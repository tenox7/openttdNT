...
static void NewAircraftWndProc(Window *w, WindowEvent *e)
{
			...
			do {
				if (HASBIT(e->player_avail, _local_player)) {
					...
					if (IS_INT_INSIDE(--pos, -4, 0)) {
						DrawString(x+62, y+7, _engine_name_strings[engine_id], sel==0 ? 0xC : 0x10);
	...
	case WE_CLICK:
		...
		case 5: { /* build */
			...
			if (DoCommandByTile(w->window_number, sel_eng, 0,
				DC_EXEC | DC_MSG(STR_A008_CAN_T_BUILD_AIRCRAFT), CMD_BUILD_AIRCRAFT) == CMD_ERROR)
					return;
	...
	case WE_ON_EDIT_TEXT: {
		...
		DoCommandByTile(0, WP(w,buildtrain_d).rename_engine, 0, DC_EXEC | DC_MSG(STR_A03A_CAN_T_RENAME_AIRCRAFT_TYPE), CMD_RENAME_ENGINE);
	} break;
	}	
}
	...
	case WE_CLICK:
		...
		case 4: /* refit button */
			if (WP(w,refit_d).cargo != 0xFF) {
				Vehicle *v = &_vehicles[w->window_number];	
				if (DoCommandByTile(v->tile, v->index, WP(w,refit_d).cargo, DC_EXEC | DC_MSG(STR_A042_CAN_T_REFIT_AIRCRAFT), CMD_REFIT_AIRCRAFT) != CMD_ERROR)
					DeleteWindow(w);
			}
		  break;
		}
...
change_int:
			...
			DoCommandByTile(v->tile, v->index, mod,
				DC_EXEC | DC_MSG(STR_018A_CAN_T_CHANGE_SERVICING), CMD_CHANGE_AIRCRAFT_SERVICE_INT);
			break;
		}
		break;
	...
	case WE_ON_EDIT_TEXT: {
		...
		if (*b == 0)
			return;
		memcpy(_decode_parameters, b, 32);
		DoCommandByTile(0, w->window_number, 0, DC_EXEC | DC_MSG(STR_A031_CAN_T_NAME_AIRCRAFT), CMD_NAME_VEHICLE);
	} break;
	...
	case WE_CLICK: {
		...
		case 4: /* start stop */
			DoCommandByTile(v->tile, v->index, 0,
				DC_EXEC | DC_MSG(STR_A016_CAN_T_STOP_START_AIRCRAFT), CMD_START_STOP_AIRCRAFT); 
			break;
		...
		case 6: /* goto hangar */
			DoCommandByTile(v->tile, v->index, 0,
				DC_EXEC | DC_MSG(STR_A012_CAN_T_SEND_AIRCRAFT_TO), CMD_SEND_AIRCRAFT_TO_HANGAR);
			break;
		case 7: /* refit */
		...
		case 3:
			if (!HASBIT(w->disabled_state, 3) &&
				...
				if (DoCommandByTile(v->tile, v->index, 0, 
					DC_EXEC | DC_MSG(STR_A01C_CAN_T_SELL_AIRCRAFT), CMD_SELL_AIRCRAFT) == CMD_ERROR)
						_backup_orders_tile = 0;
			}
			break;
		default:
