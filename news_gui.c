...
static void NewsWindowProc(Window *w, WindowEvent *e)
{
	...
	case WE_PAINT: {
		...
		if (ni->display_mode == NM_NORMAL || ni->display_mode == NM_THIN) {
			...
			DrawStringRightAligned(429, 0, STR_01FF, 0);
			...
			if (!(ni->flags & NF_VIEWPORT)) {
				...
				/* Shade the viewport into gray */
				vp = w->viewport;
				GfxFillRect(vp->left - w->left, vp->top - w->top, vp->left - w->left + vp->width - 1, vp->top - w->top + vp->height - 1, 0x4323);
...
void AddNewsItem(StringID string, uint32 flags, uint16 data_a, uint16 data_b)
{
	NewsItem *ni;
...
static void MoveNewsItems()
{
	...
	for(ni=_active_news_items + 10; ni != _active_news_items + 20; ni++) {
		if (ni->string_id == 0) {
			*ni = _news_items[0];
			memcpy(_news_items, _news_items+1, sizeof(_news_items) - sizeof(_news_items[0]) * 1);
...
void ExpireNewsItem()
{
	memcpy(_active_news_items, _active_news_items + 1, sizeof(_active_news_items) - sizeof(_active_news_items[0]));
