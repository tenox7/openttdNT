...
bool CheckPlayerHasMoney(int32 cost)
{
	if (cost > 0) {
		...
		if (pid < 8 && cost > DEREF_PLAYER(pid)->player_money) {
			SET_DPARAM32(0, cost);
...
void GetNameOfOwner(byte owner)
{
	SET_DPARAM8(2, owner);
	...
	if ((owner & 0x80) == 0) {
		...
		City *city = DEREF_CITY((int)owner - 0x80);
		SET_DPARAM16(0, city->citynametype);
		SET_DPARAM32(1, city->citynameparts);
	}
}
...
static void GenerateCompanyName(Player *p)
{
	...
	if (IS_INT_INSIDE(c->citynametype, SPECSTR_TOWNNAME_START, SPECSTR_TOWNNAME_LAST+1)) {
		str = c->citynametype - SPECSTR_TOWNNAME_START + SPECSTR_PLAYERNAME_START;
		strp = c->citynameparts;
...
static byte GeneratePlayerColor()
{
	...
	FOR_ALL_PLAYERS(p) if (p->name_1 != 0) {
...
Player *DoStartupNewPlayer(bool is_ai)
{
	...
	ResetTownRatingsForPlayer(index);
...
static void MaybeStartNewPlayer()
{
	...
	n = min(_opt.diff.max_no_competitors + _num_human_players, lengthof(_players));
	p = DEREF_PLAYER(RandomRange(n));
	if (p->name_1 == 0)
...
void OnTick_Players()
{
	...
	_cur_player_tick_index = (_cur_player_tick_index + 1) % lengthof(_players);
...
static const byte _player_desc[] = {
	...
	SLE_ARR(Player,yearly_expenses,	SLE_FILE_I32|SLE_VAR_I64, 3*13),
	SLE_CONDVAR(Player,is_ai,			SLE_UINT8, 2, 255),
	SLE_END()
};

static const byte _player_economy_desc[] = {
	SLE_VAR(PlayerEconomyEntry,income,							SLE_INT32),
	SLE_VAR(PlayerEconomyEntry,expenses,						SLE_INT32),
	SLE_VAR(PlayerEconomyEntry,delivered_cargo,			SLE_INT32),
	SLE_VAR(PlayerEconomyEntry,performance_history,	SLE_INT32),
	SLE_VAR(PlayerEconomyEntry,company_value,				SLE_INT32),
	SLE_END()
};
...
static void SaveLoad_PLYR(Player *p) {
	...
	if (p->index > 0)
		p->is_ai = true; // this is wrong, but it's all i can come up with to load old games.
