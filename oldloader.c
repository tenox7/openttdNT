...
assert_compile(sizeof(OldCity) == 0x5E);
...
#define REMAP_CITY_IDX(x) (x - (0x0459154 - 0x0458EF0)) / sizeof(OldCity)
#define REMAP_CITY_PTR(x) DEREF_CITY( REMAP_CITY_IDX(x) )
...
static void FixCity(City *c, OldCity *o, int num)
{
	do {
		c->xy = o->xy;
		c->population = o->population;
		c->citynametype = o->citynametype;
		c->citynameparts = o->citynameparts;
		if (IS_INT_INSIDE(o->citynametype, 0x20C1, 0x20C2 + 1))
			c->citynametype = SPECSTR_TOWNNAME_ENGLISH;

		c->grow_counter = o->grow_counter;
		c->flags12 = o->flags12;
		memcpy(c->ratings,o->ratings,sizeof(c->ratings));
		c->have_ratings = o->have_ratings;
		c->statues = o->statues;
		c->num_houses = o->num_houses;
		c->time_until_rebuild = o->time_until_rebuild;
		c->growth_rate = o->growth_rate;
		c->new_max_pass = o->new_max_pass;
		c->new_max_mail = o->new_max_mail;
		c->new_act_pass = o->new_act_pass;
		c->new_act_mail = o->new_act_mail;
		c->max_pass = o->max_pass;
		c->max_mail = o->max_mail;
		c->act_pass = o->act_pass;
		c->act_mail = o->act_mail;
		c->pct_pass_transported = o->pct_pass_transported;
		c->pct_mail_transported = o->pct_mail_transported;
		c->new_act_food = o->new_act_food;
		c->new_act_paper = o->new_act_paper;
		c->act_food = o->act_food;
		c->act_paper = o->act_paper;
		c->road_build_months = o->road_build_months;
		c->fund_buildings_months = o->fund_buildings_months;
	} while (c++,o++,--num);
}
...
static void FixStation(Station *s, OldStation *o, int num)
{
	do {
		...
		s->station_platforms = (o->platforms & 0x7) + ((o->platforms & (7 << 3)) << 1);

		s->alpha_order = o->alpha_order;
...
{
	do {
		n->city_index = REMAP_CITY_IDX(o->city);
		n->xy = o->xy;
	} while (n++,o++,--num);
}
...
static void FixVehicle(Vehicle *n, OldVehicle *o, int num)
{
	do {
		...
		n->next_in_chain = o->next_in_chain;
		n->value = o->value;
...
static void FixPlayer(Player *n, OldPlayer *o, int num)
{
	...
	do {
		...
		n->name_2 = o->name_2;
		if (o->name_1 == 0 && x == 0) n->name_1 = STR_SV_UNNAMED;
...
void FixGameDifficulty(GameDifficulty *n, OldGameSettings *o)
{
	...
	n->number_cities = o->number_cities;
...
bool LoadOldSaveGame(const char *file)
{
	...
	FixCity(_cities, m->city_list, lengthof(m->city_list));
	...
	_opt.city_name = m->city_name_type;
	...
	_cur_town_ctr = REMAP_CITY_IDX(m->cur_city_ptr);
