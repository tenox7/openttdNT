...
static void PlaceDocks_Dock(uint tile)
{
	...
	ret = DoCommandByTile(tile, 0, 0, 
			DC_AUTO | DC_EXEC | DC_MSG(STR_9802_CAN_T_BUILD_DOCK_HERE),
			CMD_BUILD_DOCK);
...
static void PlaceDocks_Depot(uint tile)
{
	...
	ret = DoCommandByTile(tile, _ship_depot_direction, 0, 
			DC_AUTO | DC_EXEC | DC_MSG(STR_3802_CAN_T_BUILD_SHIP_DEPOT),
...
static void PlaceDocks_Buoy(uint tile)
{
	...
	ret = DoCommandByTile(tile, 0, 0, 
			DC_AUTO | DC_EXEC | DC_MSG(STR_9835_CAN_T_POSITION_BUOY_HERE),
			CMD_BUILD_BUOY);
...
static void BuildDocksWndProc(Window *w, WindowEvent *e)
{
	switch(e->event) {
	case WE_PAINT:
		DrawWindowWidgets(w);
...
{    WWT_CAPTION,     7,    11,   153,     0,    13, STR_9801_DOCK_CONSTRUCTION,	STR_018C_WINDOW_TITLE_DRAG_THIS},
{      WWT_PANEL,     7,     0,    21,    14,    35, 0x2EA,												STR_981D_BUILD_SHIP_DOCK},
{      WWT_PANEL,     7,    22,    43,    14,    35, 0x2EC,												STR_981E_BUILD_SHIP_DEPOT_FOR_BUILDING},
{      WWT_PANEL,     7,    44,    65,    14,    35, 0x2B5,												STR_9834_POSITION_BUOY_WHICH_CAN},
{      WWT_PANEL,     7,    66,    87,    14,    35, 0x2BF,												STR_018D_DEMOLISH_BUILDINGS_ETC},
{      WWT_PANEL,     7,    88,   109,    14,    35, 0x2B7,												STR_018E_LOWER_A_CORNER_OF_LAND},
{      WWT_PANEL,     7,   110,   131,    14,    35, 0x2B6,												STR_018F_RAISE_A_CORNER_OF_LAND},
{      WWT_PANEL,     7,   132,   153,    14,    35, 0x12B7,											STR_0329_PURCHASE_LAND_FOR_FUTURE},
{      WWT_LAST},
};

static const WindowDesc _build_docks_toolbar_desc = {
	0x1E6, 0x16, 0x9A, 0x24,
	WC_BUILD_TOOLBAR,0,
