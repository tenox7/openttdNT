...
static byte *DecodeString(byte *buff, const byte *str)
{
	...
	while((b = *str++) != 0) {
		...
		case 0x9D: { // {CHECKPOINT}
			...
			StringID str;
			int idx;
			if (~cp->city_or_string & 0xC000) {
				GetParamInt32(); // skip it
				str = cp->city_or_string;
			} else {
				idx = (cp->city_or_string >> 8) & 0x3F;
				if (idx == 0) {
				...
				SET_DPARAM16(0, cp->city_or_string & 0xFF);
			}
...
static byte *GetSpecialCityNameString(byte *buff, int ind) {
	...
	_city_name_generators[ind](buff, x);
...
#define HDR ((LanguagePackHeader*)lang_pack)
	lang_pack = ReadFileToMem(_dynlang.ent[lang_index].file, &len, 100000);
...
void InitializeLanguagePacks()
{
	...
	if (!ReadLanguagePack(j))
		error("can't read language pack '%s'", dl->ent[j].file);
}
