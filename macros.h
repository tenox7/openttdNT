...
#define TILE_FROM_XY(x,y) (((((y) & ~0xF) << TILE_X_BITS) + (x)) >> 4)
...
#if defined(NDEBUG) || TILE_X_BITS != 8
...
#define TILE_MASK(x) (uint)((x) & ((1 << (TILE_X_BITS + TILE_Y_BITS))-1))
#endif
...
static INLINE void WRITE_LE_UINT16(const void *b, uint16 x) {
	...
	static INLINE uint32 READ_LE_UINT32(const void *b) {
		return ((byte*)b)[0] | (((byte*)b)[1] << 8) |
			(((byte*)b)[2] << 16) | (((byte*)b)[3] << 24);
	}
	static INLINE void WRITE_LE_UINT32(void *b, uint32 x) {
		((byte*)b)[0] = (byte)x;
		((byte*)b)[1] = (byte)(x >> 8);
		((byte*)b)[2] = (byte)(x >> 16);
		((byte*)b)[3] = (byte)(x >> 24);
...
#else
#	define READ_LE_UINT32(b) (*(uint32*)(b))
#	define WRITE_LE_UINT32(b,x) (*(uint32*)(b) = (x))
#endif
