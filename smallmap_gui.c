...
{     WWT_IMGBTN,    13,     0,   357,   258,   301, 0x0,0},
...
{     WWT_IMGBTN,    13,   226,   247,   146,   167, 0xFED,0},
...
static void DoShowSmallMap(bool big);
...
void DrawSmallMapContours(byte *dst, uint xc, uint yc, int pitch, int reps, uint32 mask)
{
	uint tile;
	int t;
	...
	do {
		if (xc < 255 && yc < 255) {
			...
			WRITE_PIXELS_OR(dst, ((_map_height_bits[_map_type_and_height[tile] & 0xF] & 
								_smallmap_contours_andor[t][1]) | _smallmap_contours_andor[t][0]) & mask);
		}
	} while (xc++,yc++,dst+=pitch,--reps != 0);
}

void DrawSmallMapVehicles(byte *dst, uint xc, uint yc, int pitch, int reps, uint32 mask)
{
	uint tile;
	int t;
	...
	do {
		if (xc < 255 && yc < 255) {
			...
			WRITE_PIXELS_OR(dst, ((MKCOLOR(0x54545454) & _smallmap_vehicles_andor[t][1]) | _smallmap_vehicles_andor[t][0]) & mask);
		}
	} while (xc++,yc++,dst+=pitch,--reps != 0);
}
...
void DrawSmallMapIndustries(byte *dst, uint xc, uint yc, int pitch, int reps, uint32 mask)
{
	uint tile;
	int t;
	uint32 bits;
	...
	do {
		if (xc < 255 && yc < 255) {
			...
			if (t == MP_INDUSTRY) {
				...
				bits = color + (color << 8) + (color << 16) + (color << 24);
			} else {
				if (t == MP_TUNNELBRIDGE) {
					t = _map5[tile];
					if ((t & 0x80) == 0) t>>=1;
			...
			WRITE_PIXELS_OR(dst, bits & mask);
		}
	} while (xc++,yc++,dst+=pitch,--reps != 0);
}

void DrawSmallMapRoutes(byte *dst, uint xc, uint yc, int pitch, int reps, uint32 mask)
{
	uint tile;
	int t;
	uint32 bits;
	...
	do {
		if (xc < 255 && yc < 255) {
			...
			if (t == MP_STATION) {
				byte m5 = _map5[tile];
				(bits = MKCOLOR(0x56565656), m5 < 8) ||
				(bits = MKCOLOR(0xB8B8B8B8), m5 < 0x43) ||
				(bits = MKCOLOR(0xC2C2C2C2), m5 < 0x47) ||
				(bits = MKCOLOR(0xBFBFBFBF), m5 < 0x4B) ||
				(bits = MKCOLOR(0x98989898), m5 < 0x53) ||
				(bits = MKCOLOR(0xFFFFFFFF), true);
			} else {
				if (t == MP_TUNNELBRIDGE) {
					t = _map5[tile];
					if ((t & 0x80) == 0) t>>=1;
...
void DrawSmallMapVegetation(byte *dst, uint xc, uint yc, int pitch, int reps, uint32 mask)
{
	uint tile;
	int t;
	uint32 bits;
...
void DrawSmallMapOwners(byte *dst, uint xc, uint yc, int pitch, int reps, uint32 mask)
{
	uint tile;
	int t;
	...
	do {
		if (xc < 255 && yc < 255) {
			...
			WRITE_PIXELS_OR(dst, _owner_colors[t] & mask);
		}
	} while (xc++,yc++,dst+=pitch,--reps != 0);
}
...
skip_column:		
	...
	if (show_towns) {
		...
		FOR_ALL_CITIES(city) {
			if (city->xy != 0) {
				// Remap the city coordinate
				Point pt = RemapCoords(
					(int)(GET_TILE_X(city->xy)*16 - WP(w,smallmap_d).scroll_x) >> 4,
					(int)(GET_TILE_Y(city->xy)*16 - WP(w,smallmap_d).scroll_y) >> 4,
					0);
				x = pt.x - WP(w,smallmap_d).subscroll + 3 - (city->sign.width_2 >> 1);
				y = pt.y;
								
				// Check if the city sign is within bounds
				if (x + city->sign.width_2 > dpi->left &&
...
static void SmallMapWindowProc(Window *w, WindowEvent *e)
{
	...
	case WE_PAINT: {
		...
		if (!FillDrawPixelInfo(&new_dpi, NULL, 3, 17, w->width - 28 + (w->widget == _smallmap_big_widgets ? 22 : 0), w->height - 64))
			return;

		DrawSmallMap(&new_dpi, w, _smallmap_type, (w->click_state & (1 << 11)) != 0);
	} break;
	...
	case WE_CLICK:
		...
		case 2: {/* big/small size */
			...
			DeleteWindow(w);
			SndPlayFx(0x13);
			DoShowSmallMap(wi == _smallmap_small_widgets);
		} break;
...
static void DoShowSmallMap(bool big)
{
	...
	w = AllocateWindowDescFront(big ? &_smallmap_big_desc : &_smallmap_small_desc, 0);
	if (w) {
		w->click_state = (1<<5) << _smallmap_type | 1 << 11;
		vp = FindWindowById(WC_MAIN_WINDOW, 0)->viewport;

		x = (((vp->virtual_width - (big?7040:3520)) >> 1) + vp->virtual_left) >> 2;
		y = ((((vp->virtual_height- (big?3840:2400)) >> 1) + vp->virtual_top) >> 1) - 32;
