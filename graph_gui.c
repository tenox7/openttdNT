...
static void SetupGraphDrawerForPlayers(GraphDrawer *gd)
{
	...
	FOR_ALL_PLAYERS(p) {
		if (p->name_1 == 0) CLRBIT(showbits,p->index);
	}	
	gd->sel = showbits;
	...
	nums = 0;
	FOR_ALL_PLAYERS(p) {
		if (p->name_1 != 0) nums = max(nums,p->num_valid_stat_ent);
	}
	...
	case WE_PAINT: {
		...
		FOR_ALL_PLAYERS(p) {
			...
			for(j=gd.num_on_x_axis,i=0; --j >= 0;) {
				gd.cost[numd][i] = (j >= p->num_valid_stat_ent) ? 0x80000000 : 
					-(p->old_economy[j].income + p->old_economy[j].expenses);
				i++;
			}
			numd++;
		}
...
static void CargoPaymentRatesWndProc(Window *w, WindowEvent *e)
{
	...
	case WE_PAINT: {
		...
		w->click_state = WP(w,paymentrates_d).sel << 3;
		DrawWindowWidgets(w);
		...
		gd.sel = ~WP(w,paymentrates_d).sel;
		gd.left = 2;
		gd.top = 24;
		gd.height = 104;
		gd.include_neg = false;
	...
	case WE_CLICK: {
		...
		case 11: case 12: case 13: case 14:
			WP(w,paymentrates_d).sel ^= 1 << (e->click.widget - 3);
			SetWindowDirty(w);
			break;
		}
	} break;
...
void ShowCargoPaymentRates()
{
	...
	w = AllocateWindowDescFront(&_cargo_payment_rates_desc, 0);
	if (w)
		WP(w,paymentrates_d).sel = 1;
}
