...
static void SetDisasterVehiclePos(Vehicle *v, int x, int y, byte z)
{
	...
	if (v->next_in_chain != INVALID_VEHICLE) {
		...
		u->x_pos = x;
		u->y_pos = yt = y - 1 - (max(z - (byte)GetSlopeZ(x, y-1), 0) >> 3);
...
static void DisasterTick_2(Vehicle *v)
{
	...
	if (v->next_order == 2) {
		if (!(v->tick_counter&3)) {
			Industry *i = &_industries[v->dest_tile];
		...
		if ((&_industries[ind])->type == IT_OIL_REFINERY) {
			v->next_order = 1;
			v->age = 0;
		}
	}
...
static void DisasterTick_3(Vehicle *v)
{
	...
	if (v->next_order == 2) {
		...
		if ((&_industries[ind])->type == IT_FACTORY) {
			v->next_order = 1;
			v->age = 0;
		}
	}
...
static void DisasterTick_4(Vehicle *v)
{
	...
	if (v->next_order == 1) {
		...
		z = (byte)GetSlopeZ(v->x_pos, v->y_pos);
		if (z < v->z_pos) {
		...
		city = ClosestCityFromTile(v->dest_tile, (uint)-1);
