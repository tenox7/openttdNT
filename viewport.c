		...
		FOR_ALL_CITIES(ci) {
			if (ci->xy &&
			    bottom > ci->sign.top &&
					top < ci->sign.top + 12 &&
					right > ci->sign.left &&
					left < ci->sign.left + ci->sign.width_1) {
				
				AddStringToDraw(ci->sign.left + 1, ci->sign.top + 1, STR_2001, ci->citynametype, ci->citynameparts);
			}
		}
	} else if (dpi->zoom == 1) {
		right += 2;
		...
		FOR_ALL_CITIES(ci) {
			if (ci->xy &&
			    bottom > ci->sign.top &&
					top < ci->sign.top + 24 &&
					right > ci->sign.left &&
					left < ci->sign.left + ci->sign.width_1*2) {
		...
		FOR_ALL_CITIES(ci) {
			if (ci->xy &&
			    bottom > ci->sign.top &&
					top < ci->sign.top + 24 &&
					right > ci->sign.left &&
					left < ci->sign.left + ci->sign.width_2*4) {
				
				AddStringToDraw(ci->sign.left + 5, ci->sign.top + 1, STR_2002, ci->citynametype, ci->citynameparts);
				AddStringToDraw(ci->sign.left + 1, ci->sign.top - 3, STR_2003, ci->citynametype, ci->citynameparts);
			}				
		}
	}
}
	...
	if (dpi->zoom < 1) {
		FOR_ALL_STATIONS(st) {
			if (st->xy &&
				...
				if (sstd != NULL) {
					sstd->color = (st->owner == 0x10 || !st->facilities) ? 0xE : _player_colors[st->owner];
...
void UpdateViewportPosition(Window *w)
{
	...
	if (WP(w,vp_d).follow_vehicle != 0xFFFF) {
		...
		t = (-130) << vp->zoom;
		err = 0;
		if (y < t || y > (t += 0xFEF)) { y = t; err++; }
		if (x < (t=0) || x > (t=0xFEF) ) { x = t; err++; }
#else
		t = x;
		x = x + y;
		y = x - y;
...
void MarkTileDirty(int x, int y)
{
	int z = 0;
	Point pt;
	if (IS_INT_INSIDE(x, 0, TILES_X*16) && IS_INT_INSIDE(y, 0, TILES_X*16))
		...
		FOR_ALL_CITIES(c) {
			if (c->xy &&
			    y >= c->sign.top &&
					y < c->sign.top + 24 &&
					x >= c->sign.left &&
					x < c->sign.left + c->sign.width_1 * 2) {
				ShowTownViewWindow(c->index);
				return true;
			}
		}
		...
		FOR_ALL_CITIES(c) {
			if (c->xy &&
			    y >= c->sign.top &&
					y < c->sign.top + 24 &&
					x >= c->sign.left &&
					x < c->sign.left + c->sign.width_2 * 4) {
				ShowTownViewWindow(c->index);
				return true;
			}
		}
...
{
	TileHighlightData *thd = _thd_ptr;
	thd->new_size.x = thd->new_maxsize.x = w * 16;
	thd->new_size.y = thd->new_maxsize.y = h * 16;
}
...
void SetTileSelectBigSize(int ox, int oy, int sx, int sy) {
	...
	thd->offs.x = ox * 16;
	thd->offs.y = oy * 16;
	thd->new_maxsize.x += sx * 16;
	thd->new_maxsize.y += sy * 16;
}
