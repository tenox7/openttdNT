...
#define SPRITECACHE_ID 0xF00F0005
...
static void ReadSpriteHeaderSkipData(int num)
{
	byte type;
	int8 i;
...
#define S_DATA(x) READ_LE_UINT32(x)
#define S_DATA_W(x,v) WRITE_LE_UINT32(x,v)
	...
	while (true) {
		...
		if (size & S_FREE_MASK) {
			...
			if (!_sprite_locked[i]) {
				...
				// Move the memory 
				// NOTE: memmove should actually be used here, but since we're moving from a high address to a low address
				//  memcpy is also safe, on those compilers where memcpy moves from left to right.
				memcpy(s+S_HDRSIZE, s+S_HDRSIZE+size, sizeb - S_HDRSIZE );
...
static void DeleteEntryFromSpriteCache()
{
	...
	S_DATA_W(s - S_HDRSIZE, S_DATA(s - S_HDRSIZE) + S_FREE_MASK);
	...
	for(;;) {
		for(;;) {
			...
			if (cur_size != mem_req) {
				...
				S_DATA_W(s + mem_req, cur_size - mem_req + S_FREE_MASK);
	...
	if (!HandleCachedSpriteHeaders(_cached_filenames[_opt.landscape], true)) {
...

		{
			...
			if ((l=_sprite_page_to_load) != 0)
				LoadGrfIndexed(_landscape_filenames[l-1], _landscape_spriteindexes[l-1], (byte)i); 
		}
...
void GfxInitSpriteMem(byte *ptr, uint32 size)
{
	...
	S_DATA_W(ptr + size - S_HDRSIZE, 0);
	S_DATA_W(ptr, size - S_HDRSIZE + S_FREE_MASK);
