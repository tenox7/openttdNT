...
static int GetStationsAlphaSorted(Station **list, int start, int num, byte owner)
{
	Station *st, *best;
	int num_found = 0;
	byte min, max;
	...
	do {
		do {
			...
			best = NULL;
			FOR_ALL_STATIONS(st) {
				if (st->xy != 0 && st->owner == owner && min <= st->alpha_order && max > st->alpha_order) {
					max = st->alpha_order;
					best = st;
				}
			}
...
static void PlayerStationsWndProc(Window *w, WindowEvent *e)
{
	...
	case WE_PAINT: {
		/* count number of stations */
		{
			...
			int num = 0;
			FOR_ALL_STATIONS(st) {
				if (st->xy != 0 && st->owner == (byte)w->window_number)
					num++;
			}
			w->vscroll.count = num;
			num -= w->vscroll.cap;
			if (num < 0) num = 0;
			if ( (byte) num < w->vscroll.pos)
				w->vscroll.pos = (byte)0;
		}
...

		{
			...
			num = GetStationsAlphaSorted(st_list, w->vscroll.pos, 12, (byte)w->window_number);
			assert(num > 0);
	...
	case WE_CLICK: {
		...
		case 2: {
			uint i = (e->click.pt.y - 15)/10;
			if (i >= 12)
				return;

			if (GetStationsAlphaSorted(st_list, i + w->vscroll.pos, 1, (byte)w->window_number) != 0) {
				ScrollMainWindowToTile( st_list[0]->xy );
			}
		} break;
		}
	} break;
...
{      WWT_PANEL,    14,     0,   346,    14,   135, 0x0, STR_3057_STATION_NAMES_CLICK_ON},
{  WWT_SCROLLBAR,    14,   347,   357,    14,   135, 0x0, STR_0190_SCROLL_BAR_SCROLLS_LIST},
{      WWT_LAST},
};
...
static void StationViewWndProc(Window *w, WindowEvent *e)
{
	...
	case WE_ON_EDIT_TEXT: {
		...
		st = DEREF_STATION(w->window_number);
		DoCommandByTile(st->xy, w->window_number, 0, DC_EXEC | DC_MSG(STR_3031_CAN_T_RENAME_STATION), CMD_RENAME_STATION);
	} break;	
	}
}
