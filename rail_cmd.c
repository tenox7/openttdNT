...
static const byte _valid_tileh_slopes[2][15] = {
...
need_clear:;
		/* isnot_railway */
		ret = DoCommandByTile(tile, 0, 0, flags & ~DC_EXEC, CMD_LANDSCAPE_CLEAR);
		if (ret == CMD_ERROR)
			return CMD_ERROR;
	}

	cost = CheckRailSlope(ti.tileh, rail_bit, existing);
	if (cost == CMD_ERROR) {
...
int32 CmdRemoveSingleRail(int x, int y, uint32 flags, uint32 p1, uint32 p2)
{
	...
	if (_current_player != 0x11 && !CheckOwnership(_map_owner[tile]))
		return CMD_ERROR;
...
{
	int cityidx = ClosestCityFromTile(cp->xy, (uint)-1)->index;
	Checkpoint *cc;
	...
	// find an unused checkpoint number belonging to this city
	for(cc = _checkpoints; cc != endof(_checkpoints); cc++) {
		if (cc->xy && cc->city_or_string & 0xC000 && (cc->city_or_string & 0xFF) == cityidx)
			used_checkpoint[(cc->city_or_string >> 8) & 0x3F] = true;
	}

	for(i=0; used_checkpoint[i] && i!=lengthof(used_checkpoint)-1; i++) {}
	cp->city_or_string = 0xC000 + (i << 8) + cityidx;
}
...
int32 CmdBuildTrainCheckpoint(int x, int y, uint32 flags, uint32 p1, uint32 p2)
{
	...
	if (flags & DC_EXEC) {
		...
		if (cp->city_or_string == 0) MakeDefaultCheckpointName(cp); else RedrawCheckpointSign(cp);
...
static void DoDeleteCheckpoint(Checkpoint *cp)
{
	...
	if (~cp->city_or_string & 0xC000) DeleteName(cp->city_or_string);
	RedrawCheckpointSign(cp);
}
...
static const byte _track_inclined_tileh[] = {
	3,9,3,6,12,6,12,9,
};
...
static void DrawSignalHelper(TileInfo *ti, byte condition, uint32 image_and_pos)
{
	...
	AddSortableSpriteToDraw(sprite, x, y, 1, 1, 10, (byte)GetSlopeZ(x,y));
}
...
void DrawFoundation(TileInfo *ti, uint f)
{
	if (f < 15) {
		...
		// inclined foundation
		AddSortableSpriteToDraw(ti->tileh + 0x3DE - 1, ti->x, ti->y, 1, 1, 1, ti->z);
		ti->tileh = _track_inclined_tileh[f - 15];
...
static void ChangeSignalStates(SetSignalsData *ssd)
{
	...
	for(i=0; i!=ssd->cur; i++) {
		...
		if (_map3_hi[tile] & 1) {
			...
			// subtract for combo signals so they don't count themselves
			if (_map3_hi[tile]&2) {
				ex--;
...
uint16 GetSlopeZ_Track(TileInfo *ti) 
{
	uint z = ti->z;
	int th = ti->tileh;
...
static void TileLoop_Track(uint tile)
{
	...
	// Don't run tile loop for depots
...
static const StringID _track_tile_str[4] = {
...
{
	td->str = _track_tile_str[_map5[tile] >> 6];
